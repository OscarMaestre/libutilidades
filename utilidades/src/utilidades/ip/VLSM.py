...
class SolucionVLSM(object):
    ...
    def get_rst(self):
        ...
        definiciones=["Bits de host asignados", "IP de Red", "Máscara en decimal", "Primera IP", "Última IP", "IP de broadcast"]
        valores=[ self.num_bits_para_host, ip_red, self.nueva_red.netmask, primera_ip, ultima_ip, ip_broadcast]
        lista=[]
        for d,v in zip(definiciones, valores):
            lista.append("* {0}: {1}".format(d, v))
        texto="Red con {0} hosts:\n\n".format(self.num_hosts)
        puntos="\n".join(lista)
        return texto+puntos+"\n"
...
class VLSM(object):
    ...
    def resolver(self):
        ...
        for numero in lista_ordenada:
            ...
            ip_red_anterior=self.generar_red(ip_red_anterior, nuevos_bits_de_red)
            redes_solucion.append(SolucionVLSM(nueva_red=ip_red_anterior, num_hosts=numero, num_bits_para_host=bits_de_host))
