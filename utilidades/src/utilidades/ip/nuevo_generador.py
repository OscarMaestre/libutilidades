...
class Generador(object):
    ...
    def convertir_mascara_cidr_a_decimal(cantidad_bits):
        secuencia_binaria=Generador.convertir_mascara_cidr_a_binario(cantidad_bits)
        mascara_con_puntos=Generador.convertir_binario_a_ip(secuencia_binaria)
        return mascara_con_puntos

    @staticmethod 
    def generar_direccion_de_red_de_clase_a():
        #Generamos 31 bits al azar y ponemos como primer bit el 0
        secuencia=Generador.generar_secuencia_binaria(31)
        secuencia="0"+secuencia
        bits_mascara=randint(6, 27)
        return (secuencia, bits_mascara)
    
    @staticmethod 
    def generar_direccion_de_red_de_clase_b():
        #Generamos 30 bits al azar y ponemos como primeros bits "10"
        secuencia=Generador.generar_secuencia_binaria(30)
...
class Tests(unittest.TestCase):
    ...
    def test_generacion_clase_a(self):
        (direccion_clase_a, bits_red)=Generador.generar_direccion_de_red_de_clase_a()
        direccion_clase_a=Generador.convertir_binario_a_ip(direccion_clase_a)
        cadena="{0}/{1}".format(direccion_clase_a, bits_red)
        #print(cadena)
    def test_generacion_clase_b(self):
        (direccion_clase_b, bits_red)=Generador.generar_direccion_de_red_de_clase_b()
        direccion_clase_b=Generador.convertir_binario_a_ip(direccion_clase_b)
        cadena="{0}/{1}".format(direccion_clase_b, bits_red)
        #print(cadena)
    def test_generacion_clase_c(self):
        (direccion_clase_c, bits_red)=Generador.generar_direccion_de_red_de_clase_b()
        direccion_clase_c=Generador.convertir_binario_a_ip(direccion_clase_c)
        cadena="{0}/{1}".format(direccion_clase_c, bits_red)
        #print(cadena)
    def test_convertir_cidr_a_binario(self):
        secuencia="1"*32
        bits=32
        secuencia_convertida=Generador.convertir_mascara_cidr_a_binario(bits)
    ...
    def test_convertir_mascara_a_decimal(self):
        mascara_calculada=Generador.convertir_mascara_cidr_a_decimal(32)
        mascara_esperada="255.255.255.255"
        self.assertEqual(mascara_calculada, mascara_esperada)

        mascara_calculada=Generador.convertir_mascara_cidr_a_decimal(24)
        mascara_esperada="255.255.255.0"
        self.assertEqual(mascara_calculada, mascara_esperada)

        mascara_calculada=Generador.convertir_mascara_cidr_a_decimal(16)
        mascara_esperada="255.255.0.0"
        self.assertEqual(mascara_calculada, mascara_esperada)

        mascara_calculada=Generador.convertir_mascara_cidr_a_decimal(18)
